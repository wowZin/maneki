...
import { Card, Table, Input, Button, Space, Tag, Popconfirm, message, Row, Col, Tooltip, Modal, Form, TimePicker, Divider, Switch } from 'antd'
import { SearchOutlined, ReloadOutlined, DeleteOutlined, TeamOutlined, EyeOutlined, SettingOutlined, PlusOutlined, MinusCircleOutlined , InfoCircleOutlined } from '@ant-design/icons'
...
const HotMoneyManagement: React.FC = () => {
  ...
  const handleOpenSettings = async () => {
    ...
    try {
      ...
      if (result.data) {
        const fixedTimes = (result.data.fixed_times || ['06:00']).map((t: string) => dayjs(t, 'HH:mm'))
        settingsForm.setFieldsValue({ enabled: result.data.enabled !== false, fixed_times: fixedTimes })
      }
    } catch {
      settingsForm.setFieldsValue({ enabled: true, fixed_times: [dayjs('06:00', 'HH:mm')] })
    } finally {
  ...
  const handleSaveSettings = async (values: any) => {
    try {
      ...
      await hotMoneyApi.saveSyncSettings({ enabled: values.enabled, fixed_times: fixedTimes })
  ...
  const renderOrganizations = (orgsJson: string) => {
    ...
    try {
      ...
      if (Array.isArray(orgs) && orgs.length > 0) {
        return (
          <Space size={[0, 4]} wrap>
            {orgs.slice(0, 5).map((org: string, idx: number) => <Tag key={idx} color="blue">{org}</Tag>)}
  ...
  const columns: ColumnsType<HotMoneyItem> = [
    { title: '游资名称', dataIndex: 'name', key: 'name', width: 180, render: (text: string) => <strong>{text}</strong> },
    { title: '描述', dataIndex: 'description', key: 'description', ellipsis: true, render: (text: string) => <Tooltip title={text}><span>{text || '-'}</span></Tooltip> },
    { title: '关联机构', dataIndex: 'organizations', key: 'organizations', width: 280, render: (text: string) => renderOrganizations(text) },
    { title: '数据来源', dataIndex: 'source', key: 'source', width: 120 },
    {
      ...
      render: (_: any, record: HotMoneyItem) => (
        <Space size="small">
          ...
          <Popconfirm title="确认删除" description="确定要删除这条游资名录数据吗？" onConfirm={() => handleDelete(record.id)}>
      ...
      >
        <Row gutter={[12, 12]} className="mb-5">
          <Col xs={24} sm={12} md={8}>
            <Input placeholder="搜索游资名称" value={searchName} onChange={e => setSearchName(e.target.value)} prefix={<SearchOutlined />} allowClear onPressEnter={() => fetchData(1, pagination.pageSize)} />
          </Col>
          <Col xs={24} sm={12} md={8}>
            <Space>
              ...
              <Button onClick={() => { setSearchName(''); fetchData(1, pagination.pageSize) }}>重置</Button>
            </Space>
          </Col>
        </Row>
        ...
        <Table rowKey="id" columns={columns} dataSource={data} loading={loading}
          ...
          onChange={handleTableChange} rowSelection={rowSelection} scroll={{ x: 1000 }}
        />
      </Card>

      <Modal title="游资名录同步设置" open={settingsModalVisible} onOk={settingsForm.submit} onCancel={() => setSettingsModalVisible(false)} confirmLoading={settingsLoading} width={600}>
        <Form form={settingsForm} layout="vertical" onFinish={handleSaveSettings}>
          ...
          <Form.Item label="固定获取时间" required tooltip="设置每天自动获取游资名录数据的时间点">
