...
const AntiArbitrageRulePage: React.FC = () => {
  ...
  const handleOpenCreate = () => {
    ...
    form.setFieldsValue({
      ...
      action: 'block',
      priority: 10,
      rule_params: { window_hours: 24, max_count: 3 },
    })
    setDrawerOpen(true)
  }
  ...
  const handleSubmit = async (values: any) => {
    try {
      const payload = {
        ...
        rule_params: values.rule_params,
      }
  ...
  const renderRuleParamsForm = () => {
    switch (strategyType) {
      case 'ip_freq':
        return (
          <>
            <Form.Item label="时间窗口（小时）" name={['rule_params', 'window_hours']}>
              <InputNumber min={1} style={{ width: '100%' }} />
            </Form.Item>
            <Form.Item label="最大次数" name={['rule_params', 'max_count']}>
              <InputNumber min={1} style={{ width: '100%' }} />
            </Form.Item>
          </>
        )
      case 'new_user_threshold':
        return (
          <>
            <Form.Item label="时间窗口（天）" name={['rule_params', 'window_days']}>
              <InputNumber min={1} style={{ width: '100%' }} />
            </Form.Item>
            <Form.Item label="最大订阅数" name={['rule_params', 'max_subscriptions']}>
              <InputNumber min={1} style={{ width: '100%' }} />
            </Form.Item>
          </>
        )
  ...
  const columns = [
    ...
    { title: '规则名称', dataIndex: 'name' },
    {
      ...
      render: (v: string) => <Tag>{strategyTypeMap[v] || v}</Tag>,
    },
    {
      title: '处理动作',
      dataIndex: 'action',
      render: (v: string) => {
        const config = actionMap[v]
  ...
  return (
    <div>
      ...
      <Drawer
        title={editingRule ? '编辑防套利规则' : '新增防套利规则'}
        width={500}
        open={drawerOpen}
